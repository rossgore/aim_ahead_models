...
## Medicaid Policy Simulation (CSV-Driven)

### Goal

The Medicaid policy simulation extends the synthetic population generated by `model.py` to explore how different Medicaid coverage policies affect insurance status and coverage loss in the same synthetic population.
...
- Outputs updated insurance status and a comparison summary across scenarios.
...
The Medicaid simulator expects the **output of `model.py`** as its input. That CSV must include at least the following columns (all provided by the pipeline):
...
These are created in Stage 1 (synthetic population) and Stage 2 (insurance status) of `model.py`.

In addition, each **policy scenario** is defined in its own CSV file with the following columns:
...
Below are examples showing how to encode the three original hard-coded scenarios as CSVs.
...
Interpretation: Any individual who is inferred as Medicaid (`Medicaid_Status == True`) and whose income is between 100 and 138% FPL loses coverage and becomes `Uninsured`.
...
Interpretation: Among Medicaid enrollees aged 18–55, 8% are randomly disenrolled (churned to `Uninsured`).
...
Interpretation: Among Medicaid enrollees, individuals flagged by the `Immigrant_Proxy` action lose coverage (probability depends on `Race_Ethnicity`).
